...
public class Informer{
    ...
    private void prepareBaseDir() {
        baseDirectory = new File(basePath + "/" + dNow().getYear() + dNow().monthOfYear().get());
        if (!baseDirectory.exists()) baseDirectory.mkdir();
    }
    
    private void prepareLogDir() {
        logDirectoryPath = basePath + "/" + dNow().getYear() + "-" + dNow().monthOfYear().get();
        logDirectory = new File(logDirectoryPath);
        if (!logDirectory.exists()) logDirectory.mkdir();
    }
    
    private void prepareInformer() throws IOException {
        informerPath = logDirectoryPath + "/" + dNow().getDayOfMonth() + "-" + tNow().hourOfDay().get() + ".txt";
